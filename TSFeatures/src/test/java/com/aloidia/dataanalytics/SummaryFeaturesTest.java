...
import com.aloidia.datascience.univariatefeatures.SampleEntropyFeature;
import org.junit.*;
...
public class SummaryFeaturesTest  {
    ...
    public void testNormal() {
        ...
        //Java 8 changed this value not concerned since the levels=1000
    ...
    @Test
    public void testEdgeCaseEntropy() {
    /*
    We require 10 sample when we use the SummaryFeature api -
    but if id entropy feature is called with only 1 point there is a negative index exception
     */
        TimeSeries timeSeries = new TimeSeries();
        timeSeries.addPoint(0, 1.0);
        SampleEntropyFeature entropyFeature = new SampleEntropyFeature();

        double featureVal = entropyFeature.calculateFeature(timeSeries);

        assertTrue(Double.isNaN(featureVal)  );
    }
